...
double measMomAction(field3D<double> *mom) {
...
//Wilson Action
double measAction(field3D<double> *mom, field3D<Complex> *gauge, field<Complex> *phi, bool postStep) {
  
  double action = 0.0;
  action += measMomAction(mom);
  action += measGaugeAction(gauge);
  field<Complex> *gauge2D = new field<Complex>(gauge->p);
  extract2DSlice(gauge2D, gauge, (gauge->p.Nz - 1)/2);
  if (gauge->p.dynamic) action += measFermAction(gauge2D, phi, postStep);
  delete gauge2D;
