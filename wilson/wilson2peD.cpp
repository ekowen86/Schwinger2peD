...
int main(int argc, char **argv) {
  ...
  p.meas_vt = (atoi(argv[20]) == 0 ? false : true);
  ...
  if(p.checkpoint_start > 0) {
    ...
    for(iter=0; iter<2*p.therm; iter++){
      ...
      cout << fixed << setprecision(8) << iter+1 << " "; //Iteration
      cout << t_total << " " << endl;                     //Time
      extract2DSlice(gauge2D, gauge3D, p.zCenter);
      plaq[p.zCenter] = measPlaq(gauge2D).real();
    }
  ...
  for(iter=iter_offset; iter<p.iter_hmc + iter_offset; iter++){
	...
	// plaqSum[z] += plaq[z];
      ...
      gettimeofday(&total_end, NULL);
      t_total = ((total_end.tv_sec  - total_start.tv_sec) * 1000000u + total_end.tv_usec - total_start.tv_usec) / 1.e6;
      cout << fixed << iter << " ";   //Iteration
      cout << fixed << setprecision(8);
      cout << t_total << " ";                             //Time
      cout << scientific;
      cout << plaq[p.zCenter] << " ";                  //Action
      cout << (double)top_stuck[p.zCenter]/(count*p.skip) << " " ;   //P(stuck)
      ...
      for(int z=0; z<p.Nz; z++) printf("%+d ", top_int[z]);  //T charge
      cout << endl;
	...
	fclose(fp);
      ...
      if(p.meas_wl) measWilsonLoops(gauge2D, plaq[p.zCenter], iter);
      //Pion Correlation
      if(p.meas_pc) measPionCorrelation(gauge2D, iter);
      // Vacuum Trace
      if(p.meas_vt) measVacuumTrace(gauge2D, iter);
      //-------------------------------------------------------------
#if 0
